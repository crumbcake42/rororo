...
export async function launchCreateSession(config, projectRoot, topic) {
    ...
    let output;
    try {
        const result = execSync(`claude --agent pm --model ${model} --output-format json --prompt-file "${promptFile}"`, {
            cwd: projectRoot,
    ...
    const issueMatch = output.match(/===ISSUE_START===\s*\nPIPELINE:\s*(.+)\nTITLE:\s*(.+)\nBODY:\n([\s\S]*?)\n===ISSUE_END===/);
    if (!issueMatch) {
