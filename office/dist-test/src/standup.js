...
export async function generateStandup(config, projectRoot) {
    ...
    if (config.standup.include_blocked && blocked.length > 0) {
        ...
        for (const issue of report.blockedHuman) {
            lines.push(`  - #${issue.number} — ${issue.title} (waiting on human response)`);
        }
        for (const issue of report.blockedDependency) {
            lines.push(`  - #${issue.number} — ${issue.title} (waiting on dependency)`);
        }
...
export function launchInteractiveStandup(config, projectRoot, standupData) {
    ...
    try {
        execSync(`claude --agent pm --model ${config.models.opus} --prompt-file "${promptFile}"`, { cwd: projectRoot, stdio: "inherit" });
    }
    catch {
