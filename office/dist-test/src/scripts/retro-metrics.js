...
try {
    ...
    for (const issue of issues) {
        ...
        if (status === "blocked-human")
            blockedCounts.human++;
        if (status === "blocked-dependency")
            blockedCounts.dependency++;
        if (status === "blocked-unclassified")
    ...
    const metrics = {
        ...
        blocked_rates: {
            total_blocked: blockedCounts.human +
                blockedCounts.dependency +
                blockedCounts.unclassified,
            by_category: blockedCounts,
            blocked_percentage: issues.length > 0
                ? Math.round(((blockedCounts.human +
                    blockedCounts.dependency +
                    blockedCounts.unclassified) /
                    issues.length) *
                    100)
    ...
    console.error(`Failed to compute retro metrics: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
}
//# sourceMappingURL=retro-metrics.js.map