...
import { listIssuesByLabel, getIssue, getIssueComments, setLabels, addComment, createPR, getPipelineLabel, } from "./github.js";
import { branchName, createWorktree, cleanupWorktree, } from "./worktree.js";
...
function createWorktreeFromRemote(projectRoot, branch, issueNumber) {
    ...
    execSync(`git worktree add -B "${branch}" "${worktreePath}" "origin/${branch}"`, { cwd: projectRoot, stdio: "pipe" });
    return { path: worktreePath, branch, issueNumber };
}
function getCompletedStepIndices(worktreePath) {
    try {
...
function commitStep(worktreePath, stepNumber, totalSteps, role) {
...
function pushBranchOnFailure(worktreePath, branch, baseBranch) {
    try {
...
function loadPipeline(projectRoot, pipelineName) {
    ...
    return yaml.load(readFileSync(pipelinePath, "utf-8"));
}
function assembleContext(projectRoot, issue, comments, pipeline, stepIndex) {
    const parts = [];
    ...
    if (existsSync(specsDir)) {
        const specDirs = readdirSync(specsDir, { withFileTypes: true }).filter((d) => d.isDirectory());
    ...
    parts.push(`Step ${stepIndex + 1} of ${pipeline.steps.length}: ${pipeline.steps[stepIndex].role}`);
...
function invokeAgent(config, projectRoot, worktreePath, step, contextPrompt, captureOutput = false) {
    const model = getModelForRole(config, step.role);
    const agentFile = resolve(projectRoot, ".claude", "agents", `${step.role}.md`);
    ...
    if (result.status !== 0) {
        throw new Error(`Agent ${step.role} failed with exit code ${result.status}`);
    }
    return captureOutput && result.stdout ? String(result.stdout) : "";
}
function formatTranscript(rounds) {
    const parts = ["# Adversarial Debate Transcript\n"];
...
async function runAdversarialDebate(config, projectRoot, worktreePath, issue, comments, pipeline) {
    ...
    if (!stepA || !stepB || !pmStep) {
        throw new Error("Adversarial pipeline must have instance A, instance B, and a PM step");
    }
    const directiveA = directives[stepA.directive_index ?? 0] ?? "";
    const directiveB = directives[stepB.directive_index ?? 1] ?? "";
    const baseContext = assembleContext(projectRoot, issue, comments, pipeline, 0);
    const rounds = [];
    for (let round = 1; round <= maxRounds; round++) {
        ...
        const outputA = invokeAgent(config, projectRoot, worktreePath, stepA, contextA, true);
        const contextB = [
            baseContext,
            `\n## Your Directive\n\n${directiveB}`,
        ...
        const outputB = invokeAgent(config, projectRoot, worktreePath, stepB, contextB, true);
    ...
    const synthesis = invokeAgent(config, projectRoot, worktreePath, pmStep, pmContext, true);
    await addComment(issue.number, `## PM Synthesis\n\n${synthesis}`);
    console.log("PM synthesis posted to issue.");
    if (userStep) {
        await setLabels(issue.number, ["status:blocked-human"], ["status:in-progress"]);
        await addComment(issue.number, `The adversarial debate is complete and the PM has provided a synthesis above.\n\nPlease review and respond with your decision. The architect will update ARCHITECTURE.md and the relevant OpenSpec spec to reflect the approved decision.`);
        await notify(config, {
...
export async function dispatchNext(config, projectRoot, issueNumber) {
    let issue;
    if (issueNumber) {
...
export async function dispatchIssue(config, projectRoot, issue, pipelineName) {
    ...
    try {
        ...
        if (pipeline.adversarial) {
            await runAdversarialDebate(config, projectRoot, worktree.path, issue, comments, pipeline);
            return;
        }
        ...
        for (let i = 0; i < pipeline.steps.length; i++) {
            ...
            if (step.role === "user") {
                await setLabels(issue.number, ["status:blocked-human"], ["status:in-progress"]);
                await addComment(issue.number, `Pipeline step ${i + 1} requires user input: ${step.description}\n\nPlease respond on this issue to continue.`);
                await notify(config, {
                    issueNumber: issue.number,
                    title: issue.title,
                ...
                console.log(`Blocked on user input at step ${i + 1}. Respond on the issue to continue.`);
                return;
            }
            ...
            const context = assembleContext(projectRoot, issue, comments, pipeline, i);
        ...
        await addComment(issue.number, `Pipeline failed at a step:\n\n\`\`\`\n${message}\n\`\`\``);
        await setLabels(issue.number, ["status:blocked-unclassified"], ["status:in-progress"]);
        await notify(config, {
            issueNumber: issue.number,
            title: issue.title,
