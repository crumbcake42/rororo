...
function readState(dir) {
    return JSON.parse(readFileSync(join(dir, STATE_FILE), "utf-8"));
}
describe("pause()", () => {
    let tmpDir;
    let logMock;
    ...
    test("logs confirmation message on success", () => {
        ...
        assert.ok(logged.some((msg) => String(msg).includes("paused")), `Expected confirmation in: ${JSON.stringify(logged)}`);
    });
});
describe("resume()", () => {
    let tmpDir;
    ...
    test("logs confirmation message on success", () => {
        ...
        resume(tmpDir);
        const logged = logMock.mock.calls.map((c) => c.arguments[0]);
        assert.ok(logged.some((msg) => String(msg).includes("resumed")), `Expected confirmation in: ${JSON.stringify(logged)}`);
    });
...
describe("daemonStatus()", () => {
    ...
    test("prints tasks dispatched count", async () => {
        ...
        await daemonStatus(tmpDir);
        const output = logMock.mock.calls.map((c) => String(c.arguments[0])).join("\n");
        assert.ok(output.includes("3"), `Expected tasks count '3' in output: ${output}`);
    });
    ...
    test("shows formatted timestamp when last dispatch exists", async () => {
        ...
        assert.ok(!lastDispatchLine.includes("never"), `Last dispatch line should not say 'never': ${lastDispatchLine}`);
    });
